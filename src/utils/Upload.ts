...
export const absoluteFileSystemUploadPath = path.join(
  absoluteFileSystemDataPath + publicUploadPath
);
export const relativeFileSystemDataPath = path.join('./data');
export const relativeFileSystemUploadPath = path.join(
  relativeFileSystemDataPath + publicUploadPath
);
...
export function getUploadTarget(
  ...
  let filename = sanitisedName.substring(
    0,
  ...
  const targetFileName =
    filename + targetFileNameSuffix + randomStringSuffix + '.' + ext;

  const targetFilePath = relativeFileSystemUploadPath + '/' + targetFileName;
  const publicURL =
    (accessURL || LinkedFileStorage.accessURL) +
    publicUploadPath +
    '/' +
    targetFileName;
...
type UploadSingleFileFromFormData = {
  file: File;
  allowedExtensions?: string[];
  processDataFn?: (data) => any;
  addSuffix?: string;
};
export function uploadSingleFileFromFormData({
  file,
  allowedExtensions,
  processDataFn,
  addSuffix,
}: UploadSingleFileFromFormData): Promise<string> {
  let { targetFileName } = getUploadTarget(
  ...
  return new Promise(async (resolve, reject) => {
    try {
      fs.readFile(file.filepath, async (err, data) => {
        ...
        const updatePublicURL = await LinkedFileStorage.saveFile(
          targetFileName,
          data
        );
...
export function uploadSingleFileFromBuffer({
  buffer,
  fileName,
  allowedExtensions,
  addSuffix,
}: UploadSingleFileFromBufferType): Promise<string> {
  let { targetFileName } = getUploadTarget(
    fileName,
    null,
    allowedExtensions,
  ...
  return new Promise(async (resolve, reject) => {
    try {
      let updatedPublicURL = await LinkedFileStorage.saveFile(
        targetFileName,
        buffer
      );
